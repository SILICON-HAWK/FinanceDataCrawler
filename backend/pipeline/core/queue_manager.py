...
class QueueManager:
    ...
    def list_all_companies(self) -> Dict[str, list[dict]]:
        """
        List all discovered companies grouped by sector.

        Returns:
            Dictionary mapping sector names to list of {name, url}
        """
        from ..utils.helpers import extract_company_name_from_url
        companies = self.load_company_queue()
        result = {}
        for sector_name, urls in companies.items():
            result[sector_name] = [
                {"name": extract_company_name_from_url(url), "url": url}
                for url in urls
            ]
        return result

    def search_company_by_name(self, name: str) -> list[dict]:
        """
        ...
        Returns:
            List of matching {sector, name, url}
        """
        from ..utils.helpers import extract_company_name_from_url
        companies = self.load_company_queue()
        matches = []
        ...
        for sector_name, urls in companies.items():
            for url in urls:
                cname = extract_company_name_from_url(url)
                if cname and name_lower in cname.lower():
                    matches.append({"sector": sector_name, "name": cname, "url": url})
        return matches
